...
from datasets import RecWithContrastiveLearningDataset, DS, RecSoftWithContrastiveLearningDataset

from trainers import FENRecTrainer
...
def main():
    ...
    parser.add_argument("--lambda_0", type=float, default=0.1,
                        help="weight of coarse-grain intent contrastive learning task")
    ...
    parser.add_argument("--f_neg", action="store_true", default=True,
                        help="delete the FNM component (both in cicl and ficl)")

    # learning related
    parser.add_argument("--weight_decay", type=float, default=0.0, help="weight_decay of adam")

    # parser.add_argument("--mix_ratio", type=float, default=0.3, help="weight_decay of adam")
    parser.add_argument("--min_mix_ratio", type=float, default=0.4, help="weight_decay of adam")
    parser.add_argument("--max_mix_ratio", type=float, default=0.4, help="weight_decay of adam")
    parser.add_argument("--mix_ratio_rate", type=float, default=0.0, help="weight_decay of adam")
    parser.add_argument("--mix_portion", type=float, default=0.1, help="weight_decay of adam")
    parser.add_argument("--noise_negative_portion", type=float, default=2, help="weight_decay of adam")
    parser.add_argument("--tau", type=float, default=8, help="weight_decay of adam")
    parser.add_argument("--focal_m", type=float, default=0.2, help="weight_decay of adam")
    parser.add_argument("--soft_label_gamma", type=float, default=0.4, help="weight_decay of adam")
    ...
    args.train_user_seq_path = args.data_dir + args.data_name + "_train_user_seq.pickle"
    args.user_seq_path = args.data_dir + args.data_name + "_user_seq.pickle"
    ...
    # training data
    _, train_user_seq, _, _, _, _ = get_user_seqs(args.train_data_file, args.train_user_seq_path)
    # valid and test data
    _, user_seq, max_item, valid_rating_matrix, test_rating_matrix, count_dict = get_user_seqs(args.data_file,
                                                                                               args.user_seq_path)
    ...
    # training data
    train_dataset = RecSoftWithContrastiveLearningDataset(  # RecWithContrastiveLearningDataset
        args, train_user_seq, data_type="train"
    )
    ...
    trainer = FENRecTrainer(model, train_dataloader, cluster_dataloader, eval_dataloader,
                            test_dataloader, args)

    if args.do_eval:
        trainer.args.train_matrix = test_rating_matrix
        trainer.load(args.checkpoint_path)  # args.checkpoint_path
